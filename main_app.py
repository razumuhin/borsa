...
from ai_modules.lstm_predictor import PricePredictor
from ai_modules.sentiment_analyzer import NewsAnalyzer
...
class BistAnalizUygulamasi:
    def __init__(self, root):
        self.root = root
        self.analyzer = NewsAnalyzer(self.root) 
        ...
        self.analyzer = NewsAnalyzer(root)
    ...
    def __init__(self, root):
        # AI Modüllerini yükle
        self.root = root
        self.predictor = PricePredictor()
        self.analyzer = NewsAnalyzer(self.root)
        self.detector = AnomalyDetector()
        
    def _get_historical_data(self, hisse_kodu, period="1y"):
        """Hisse verisini çeker (AI analizleri için)"""
        try:
            df = yf.Ticker(f"{hisse_kodu}.IS").history(period=period)
            return df[['Close', 'Volume']].dropna()
        except Exception as e:
            messagebox.showerror("Hata", f"Veri alınamadı: {str(e)}")
            return pd.DataFrame()

    def _display_ai_results(self, predictions, sentiment, anomalies):
        """AI sonuçlarını gösterir"""
        self.text_output.config(state=tk.NORMAL)
        self.text_output.delete(1.0, tk.END)
        
        # Tahminleri göster
        self.text_output.insert(tk.END, "🔮 AI TAHMİNLERİ (Sonraki 7 Gün):\n")
        for i, pred in enumerate(predictions, 1):
            self.text_output.insert(tk.END, f"   • Gün {i}: {pred:.2f} TL\n")
        
        # Sentiment analiz
        self.text_output.insert(tk.END, "\n📰 SENTIMENT ANALİZİ:\n")
        self.text_output.insert(tk.END, f"   • Ortalama Skor: {sentiment['avg_score']:.2f}\n")
        for i, news in enumerate(sentiment['news'], 1):
            self.text_output.insert(tk.END, f"   {i}. {news[:50]}...\n")
        
        # Anomaliler
        self.text_output.insert(tk.END, "\n⚠ ANOMALİLER:\n")
        if not anomalies.empty:
            for date, row in anomalies.iterrows():
                self.text_output.insert(tk.END, f"   • {date.date()}: Fiyat={row['Close']:.2f}\n")
        else:
            self.text_output.insert(tk.END, "   • Anomali tespit edilmedi\n")
        
        self.text_output.config(state=tk.DISABLED)

    def show_ai_analysis(self):
        """Tüm AI analizlerini çalıştırır ve gösterir"""
        hisse = self.hisse_var.get().strip().upper()
        
        if not hisse:
            messagebox.showwarning("Uyarı", "Lütfen bir hisse seçin")
            return
        
        try:
            # Veriyi al
            data = self._get_historical_data(hisse)
            if data.empty:
                messagebox.showerror("Hata", "Hisse verisi alınamadı")
                return
            
            # Tahminleri yap
            predictions = self.predictor.predict(data['Close'].values.reshape(-1, 1))
            sentiment = self.analyzer.analyze(hisse)
            anomalies = self.detector.detect(data)
            
            # Sonuçları göster
            self._display_ai_results(predictions, sentiment, anomalies)
            
        except Exception as e:
            messagebox.showerror("Hata", f"AI analizi başarısız:\n{str(e)}")
    ...
    def setup_ui(self):
        ...
        ttk.Button(button_frame, text="AI Analiz", command=self.show_ai_analysis).pack(side=tk.LEFT, padx=3)
...
class PricePredictor:
    ...
    def create_model(self, data):
        ...
        model.fit(self._prepare_data(scaled_data), epochs=1, batch_size=1)
...
class NewsAnalyzer:
    def __init__(self, root):
        ...
        self.vader_analyzer = nltk.sentiment.SentimentIntensityAnalyzer()

    def get_news(self, hisse_kodu):
